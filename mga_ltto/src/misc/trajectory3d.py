...
#Old Spice direct ephemerides
    ...
    fig.tight_layout()
    if save_fig:
        fig.savefig('figures/' + f"trajectory3d_{vehicles_names[0]}.jpg", bbox_inches="tight")
...
def trajectory_2d(
    ...
    for i, vehicle_name in enumerate(vehicles_names):
        if len(vehicle_name) != 0:
            ...
            i_ls = i_ls + 1 if i_ls != len(linestyles) else 0
            # Plot the trajectory of the vehicle
            ax.plot(vehicles_positions[i][:,0] / au, vehicles_positions[i][:,1] / au, label=vehicle_name, color='k',
                    linestyle='-', linewidth=0.5)
            ax.scatter(vehicles_positions[i][0, 0] / au , vehicles_positions[i][0, 1]  / au, marker='D', s=40,
                       color='k')
            ax.scatter(vehicles_positions[i][-1, 0] / au , vehicles_positions[i][-1, 1] / au , marker='D', s=40,
                       color='k')
            ...
            if thrust_history != None:
                ax.quiver(vehicles_positions[i][:,0] / au, vehicles_positions[i][:,1] / au,
                          normalized_vehicles_thrust_array[:, 0],
                          normalized_vehicles_thrust_array[:, 1],
                          normalized_vehicles_thrust_array[:, 2], color='r', length = 0.3,
                          normalize=True, alpha=0.6)
                # ax.quiver(vehicles_positions[i][:,0] / au, vehicles_positions[i][:,1] / au,
...
#Old Spice direct ephemerides
        ...
        i_ls = i_ls + 1 if i_ls != len(linestyles) else 0
        # Plot the trajectory of the body
        ax.plot(body_state_array[:,0] / au, body_state_array[:,1] / au, label=body, color=_color, linestyle=_linestyle)
        ax.scatter(body_state_array[0, 0] / au, body_state_array[0, 1] / au, marker="o", color=_color)
        ax.scatter(body_state_array[-1, 0] / au, body_state_array[-1, 1] / au, marker="o", color=_color)

    # Plot the central body position
    ax.scatter(0.0, 0.0, label=central_body_name, marker="o", color="k")
    ...
    fig.tight_layout()
    if save_fig:
        fig.savefig('figures/' + f"trajectory2d_{vehicles_names[0]}.jpg", bbox_inches="tight")
